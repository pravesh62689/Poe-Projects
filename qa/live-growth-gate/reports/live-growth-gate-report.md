...
**Execution Timestamp:** 2026-09-07T07:55:32.633Z  
...
| :--- | :--- | :--- | :---: | :--- | :--- | :---: | :---: |
| `PROTO-LIVE-001-OCR` | ocr | Production URL | UNAUTHENTICATED | HTTP 200 { status: "ok" } | HTTP 200 {"status":"ok","service":"ocr-doc-bot"} | 511ms | **PASS** |
| `PROTO-LIVE-001-REGEX` | regex | Production URL | UNAUTHENTICATED | HTTP 200 { status: "ok" } | HTTP 200 {"status":"ok","service":"regex-bot"} | 444ms | **PASS** |
| `PROTO-LIVE-001-SQL` | sql | Production URL | UNAUTHENTICATED | HTTP 200 { status: "ok" } | HTTP 200 {"status":"ok","service":"sql-bot"} | 481ms | **PASS** |
| `PROTO-LIVE-005-OCR` | ocr | Production URL | UNAUTHENTICATED | HTTP 401 Unauthorized with 0 secret leakage | HTTP 401 (272ms) | 272ms | **PASS** |
| `PROTO-LIVE-005-REGEX` | regex | Production URL | UNAUTHENTICATED | HTTP 401 Unauthorized with 0 secret leakage | HTTP 401 (180ms) | 180ms | **PASS** |
| `PROTO-LIVE-005-SQL` | sql | Production URL | UNAUTHENTICATED | HTTP 401 Unauthorized with 0 secret leakage | HTTP 401 (158ms) | 158ms | **PASS** |
...
| :--- | :--- | :--- | :---: | :---: | :---: |
| `OCR-LIVE-001` | ocr | Vendor: QUICK MART, Total: $27.56, Confidence: high | **LOCAL_VERIFIED** | **PASS** | 15ms |
...
| `OCR-LIVE-008` | ocr | Treated prompt injection as text literal: Total $10 | **LOCAL_VERIFIED** | **PASS** | 2ms |
| `OCR-LIVE-009` | ocr | Engine validated handling edge variation gracefully | **LOCAL_VERIFIED** | **PASS** | 0ms |
| `OCR-LIVE-010` | ocr | Extracted 2 transaction rows, Closing: $2800 | **LOCAL_VERIFIED** | **PASS** | 2ms |
...
| `REGEX-LIVE-004` | regex | Static AST ReDoS flag: isSafe=false | **LOCAL_VERIFIED** | **PASS** | 1ms |
...
| `SQL-LIVE-001` | sql | Executed in SQLite sandbox: returned row id=1, name=Alice | **LOCAL_VERIFIED** | **PASS** | 80ms |
| `SQL-LIVE-002` | sql | JOIN verified: Alice total = $125 | **LOCAL_VERIFIED** | **PASS** | 12ms |
...
| `SQL-LIVE-008` | sql | Detected destructive query: Warning emitted | **LOCAL_VERIFIED** | **PASS** | 6ms |
