 ...
 */
export function cleanRawSqlQuery(raw: string): string {
 ...
 */
export function synthesizeSqlFromSchema(schema: string, ask: string): string {
  const tableMatches = [...schema.matchAll(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([a-zA-Z0-9_]+)/gi)];
  const tables = tableMatches.map((m) => m[1]).filter((t): t is string => Boolean(t));
 ...
 */
export async function queryPoeSqlBot(
  botName: string,
  systemPrompt: string,
  userPrompt: string,
  apiKey: string,
