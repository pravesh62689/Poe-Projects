...
export function checkDestructiveSql(sql) {
...
export async function handleSqlWorkerRequest(request, env = {}, customGenerator) {
    ...
    if (body.type === 'settings') {
        const settings = buildSettingsResponse({
            ...
            introductionMessage: 'Welcome! Provide your CREATE TABLE statements (+ optional sample rows) and describe the query you want. I will generate and actually execute the SQL in an in-memory database to verify it before returning the results.',
        ...
        (async () => {
            try {
                ...
                try {
                    ...
                    stream.sendText(`❌ **Schema Initialization Error**\n\nYour table definition contains a syntax or constraint error:\n\`\`\`text\n${errMsg}\n\`\`\`\nPlease review your \`CREATE TABLE\` / \`INSERT\` statements.`);
                    stream.close();
                    return;
                }
                // Candidate query generation
                const generateCandidate = async (failedSql, errorMsg) => {
                    if (customGenerator) {
                        return customGenerator(schema, ask, failedSql, errorMsg);
                    }
                    if (queryReq.api_key) {
                        const systemPrompt = 'You are a SQLite SQL query expert. Respond ONLY with a valid SQLite SQL statement. Do not include explanation.';
                ...
                if (outcome.success && outcome.result) {
                    ...
                    lines.push('', '```sql', outcome.query, '```', '', `**Execution Time**: \`${outcome.result.executionTimeMs}ms\``, '');
                    ...
                    lines.push('', '> ℹ️ *Dialect Notice: This query was validated against in-memory SQLite semantics. If targeting PostgreSQL, MySQL, or SQL Server, verify dialect-specific date, string, or window functions.*');
